...
describe("clampCenter", () => {
  it("returns the same point when inside bounds", () => {
    const point = { longitude: CAMPUS.center.longitude, latitude: CAMPUS.center.latitude };
...
describe("isInsideBounds", () => {
  ...
  it("returns false for point outside west", () => {
    expect(isInsideBounds({ longitude: 119.0, latitude: CAMPUS.center.latitude })).toBe(false);
  });

  it("returns false for point outside north", () => {
    expect(isInsideBounds({ longitude: CAMPUS.center.longitude, latitude: 32.0 })).toBe(false);
  });
